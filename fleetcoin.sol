...
the contract */
    ...
    mapping(address => TripStruct) public trips;
        ...
        event UpdateLocation (address _driver, string _location, TripState _state, string _pickup, string _dropoff, string _altitude, string _velocity, uint _timestamp, string description);
    

function get_status(address driver) public view returns(TripState _state, string _location, string _pickup, string _dropoff, string _altitude, string _velocity, uint _timestamp, string _description) {
...
return driverList.length;
	...
	if (current_state == TripState.READY && new_state == TripState.INPROGRESS) {
        ...
        // create public event on blockchain with dropoff = empty string
        emit UpdateLocation (msg.sender, location, TripState.READY, location, "", altitude, velocity, timestamp, description);
	}
	...
	if (current_state == TripState.INPROGRESS && new_state == TripState.INPROGRESS) {
		...
		// create public event on blockchain with pickup and dropoff empty strings 
        emit UpdateLocation (msg.sender, location, TripState.INPROGRESS, "", "", altitude, velocity, timestamp, description);
	}
	...
	if (current_state == TripState.INPROGRESS && new_state == TripState.READY) {
        ...
        // create public event on blockchain with pickup = empty string 
        emit UpdateLocation (msg.sender, location, TripState.READY, "", location, altitude, velocity, timestamp, description);
	}
