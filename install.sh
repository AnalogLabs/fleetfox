...
Your choice> " choice
	...
	sudo rm -r $PWD/simbel/data_privnet/geth

	if [ ! -d $PWD/simbel ]; then
	    mkdir -p $PWD/simbel;
	    mkdir -p $PWD/simbel/data_privnet;
	...
	./geth --datadir=$PWD/simbel/data_privnet init $PWD/simbel/genesis.json
...
echo -e "\033[1;32mCongratulations! Simbel has been installed on your machine. \033[0m"
	...
	echo Clearing chain data from $PWD/simbel/data_privnet/geth..
	sudo rm -r $PWD/simbel/data_privnet/geth
	echo Initializing private network with default genesis block.
	./geth --datadir=$PWD/simbel/data_privnet init $PWD/simbel/genesis.json
